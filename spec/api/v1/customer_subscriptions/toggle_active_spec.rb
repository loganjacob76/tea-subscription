...
RSpec.describe 'customer_subscriptions controller, create method' do
  ...
  describe 'you can create a new customer subscription' do
    it 'returns email & api_key not password' do
      patch "/api/v1/customer_subscriptions/toggle_active?customer_id=#{@customer.id}&subscription_id=#{@subscription.id}"
      ...
      expect(rdata[:attributes][:active]).to eq(false)
  ...
  describe 'SAD Path' do
    it 'one param is missing' do
      patch "/api/v1/customer_subscriptions/toggle_active?customer_id=#{@customer.id}"
