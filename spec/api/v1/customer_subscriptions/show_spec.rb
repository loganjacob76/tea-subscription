...
RSpec.describe 'customer_subscriptions controller, create method' do
  before :each do
    ...
    CustomerSubscription.create(customer: @customer, subscription: @subscription2)
  end
  ...
  describe 'SAD Path' do
    it 'invalid customer id' do
      get "/api/v1/customer_subscriptions/fake_id"
      ...
      expect(error[:errors]).to eq('Existing customer ID must be present')
